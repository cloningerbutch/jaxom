...
exports.handler = argv => {
    ...
    ipc.connectTo(id,function(){
        ...
        ref.on('disconnect',function(){
            if (!found){
                spinner.stop()
                console.log(util.format('status: %s',chalk.red('not running')));
            }
            process.exit(0);
        })
        ref.on('ack',function(){
        ...
        ref.on('status',function(dat){
            ...
            console.log(util.format('Transactions: total: %d, success: %d (%d%%), fail: %d, bandwidth: %s',
                //fmt.format(dat.stats.total),
                dat.stats.total,
                dat.stats.success,
                fmtpct.format(dat.stats.success / dat.stats.total * 100),
                dat.stats.fail,
                jutil.sizeExact(dat.stats.bytes)));
