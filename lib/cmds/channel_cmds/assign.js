...
exports.command = 'assign <channel_list> [endpoint_list]';
exports.desc = 'assign channel to endpoint(s)';
...
exports.handler = async argv => {
    ...
    console.log('assigning %d channels to %d endpoints',chanlist.length,eplist.length)
    ...
    eplist.forEach(epseq => {
        ...
        chanlist.forEach(chanseq => {
            ...
            if (!ep.channels.includes(chanseq))  {
                ep.channels.push(chanseq)
                ep.channels.sort()
                changed.push(ep);
            }
        })
